...
def reset(row, workbook):
    ...
    if exch != "Binance":
        ...
        data, url = fetch_market_price(sym, "reset")
        if not data:
            return
        ...
        if prx < marketsell:
            ...
            try:
                order_detail = create_sell_order(sym, columnS)
                if order_detail:
                    print("proceeding step 6.2.3.2.2.1?")
                    send_email("Crypto-Binance-ResetDone", CLIENT.generate_reset_email(sym, order_detail, columnS, marketsell))
                ...
                send_email("Crypto-Binance-ResetOrderError", CLIENT.generate_order_error_mail(sym, e.message))
